...
function isCloudflareWorker() {
  const g = globalThis as { WebSocketPair?: unknown; navigator?: { userAgent?: string } };
  return (
    typeof g.WebSocketPair === "function" ||
    /Cloudflare-Workers/i.test(g.navigator?.userAgent ?? "")
  );
}
...
async function createSql(): Promise<Sql> {
  ...
  if (isCloudflareWorker()) {
    throw new Error("DATABASE_URL is missing on the Worker. Set it as a Secret and redeploy.");
  }
...
if (typeof window === "undefined" && !isCloudflareWorker() && getDbSource() === "pglite") {
